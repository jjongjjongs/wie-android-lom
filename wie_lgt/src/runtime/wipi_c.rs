...
/// destination geometry (`MC_grpGetFrameBuffer{Pointer,Width,Height,Bpl,Bpp}`,
/// `MC_grpGetImageFrameBuffer`) and pokes the per-op graphics stub (`0xcf`) once
/// *per pixel or per row*: on Demon Hunter's loading screen these were ~40k
/// calls/second, the near-entirety of WIPI-C traffic. Handling them here —
/// before the generic async dispatch — skips the per-call context clone plus the
/// two `async_trait` future allocations the async handler pays, which for these
/// trivial calls cost far more than the work itself and were starving the blit.
fn install_fast_wipic_svc(core: &ArmCore) {
    ...
    const ID_GET_FRAMEBUFFER_BPL: u32 = WIPICSvcId::GetFramebufferBpl as u32;
