...
impl TextBoxComponent {
    ...
    async fn insert(
        ...
        let chars: ClassInstanceRef<()> = jvm
            .invoke_virtual(
                &text,
                "toCharArray",
                "()[C",
        ...
        let cursor: i32 =
            jvm.get_field(&this, "m_cPos", "I").await?;
    ...
    async fn set_string(
        ...
        let chars: ClassInstanceRef<()> = jvm
            .invoke_virtual(&text, "toCharArray", "()[C", ())
            .await?;
    ...
    async fn check_position(
        ...
        let caret_bottom = line
            .wrapping_add(1)
            .wrapping_mul(line_height);

        let screen_y: i32 = jvm
            .invoke_virtual(
                &this,
                "getYOnScreen",
                "()I",
        ...
        if direction == 11 {
            let viewport_y: i32 = jvm
                .get_field(&this, "__wieViewportY", "I")
                .await?;

            Ok(absolute_bottom >= viewport_y)
        } else if direction == -11 {
            let viewport_height: i32 = jvm
                .get_field(&this, "__wieViewportHeight", "I")
                .await?;
            let viewport_y: i32 = jvm
                .get_field(&this, "__wieViewportY", "I")
                .await?;
            ...
            Ok(
                absolute_bottom
                    <= viewport_y.wrapping_add(viewport_height),
            )
        } else {
            Ok(true)
        }
    ...
    async fn check_scroll(
        ...
        let caret_bottom = this_y.wrapping_add(
            current_line
                .wrapping_add(1)
                .wrapping_mul(line_height),
        );

        let parent_y: i32 = jvm
            .invoke_virtual(
                &parent,
                "getYOnScreen",
                "()I",
        ...
        let target_y = if caret_bottom <= parent_y {
            caret_bottom
                .wrapping_sub(parent_y)
                .wrapping_sub(line_height)
        } else if caret_bottom
            > parent_y.wrapping_add(viewport_height)
        {
            caret_bottom
                .wrapping_sub(viewport_height)
                .wrapping_sub(parent_y)
        } else {
            0
        };
    ...
    async fn get_preferred_height_with_width(
        ...
        let parent_height: i32 = if parent.is_null() {
            0
        } else {
            jvm.get_field(&parent, "h", "I").await?
        };

        let available_height = if parent.is_null() {
            10
        } else {
            parent_height.wrapping_sub(4)
        };
        ...
        let refresh_cached_height: i32 =
            jvm.get_field(&this, "__wieTextBoxFlag98", "I").await?;
        ...
        let mut narrow_mode: i32 =
            jvm.get_field(&this, "__wieTextBoxFlag94", "I").await?;
        ...
        let mut calculated: i32 = jvm
            .invoke_virtual(
                ...
                "setWHSize",
                "(II)I",
                (base_width, available_height),
            )
            .await?;
        calculated = calculated.wrapping_add(2);

        let component_height: i32 =
        ...
        let cached: i32 =
            jvm.get_field(&this, "__wieTextBoxValue", "I").await?;

        Ok(if calculated >= cached {
            calculated
        } else {
            cached
        })
    ...
    async fn control_popup(
        ...
        let popup = jvm
            .new_class(
                "org/kwis/msp/lwc/TextPopup",
                "(Ljava/lang/String;III)V",
                (text, constraint, mode, caret),
            )
            .await?;

        let max_length: i32 = jvm
            .invoke_virtual(
                &this,
                "getMaxLength",
                "()I",
        ...
        let action: ClassInstanceRef<()> = jvm
            .get_field(
                &this,
                "__wieTextBoxAction",
                "Ljava/lang/Object;",
        ...
        let title: ClassInstanceRef<String> =
    ...
    async fn key_notify(
        ...
        if action == 8 {
            let popup_flag: i32 =
                jvm.get_field(&this, "__wieTextBoxFlag9c", "I").await?;
        ...
        let flag_a8: i32 =
            jvm.get_field(&this, "__wieTextBoxFlagA8", "I").await?;
        ...
        jvm.put_field(&mut this, "m_cPos", "I", new_position)
            .await?;

        if new_position < 0 {
            jvm.put_field(&mut this, "__wieTextBoxFlagA8", "I", 1)
                .await?;
            jvm.put_field(&mut this, "m_cPos", "I", old_position)
                .await?;
            return Ok(false);
        }
        ...
        if new_position > char_count {
            jvm.put_field(&mut this, "__wieTextBoxFlagA8", "I", 1)
                .await?;
            jvm.put_field(&mut this, "m_cPos", "I", char_count)
                .await?;
            return Ok(false);
        }

        jvm.put_field(&mut this, "__wieTextBoxFlagA8", "I", 0)
            .await?;
        ...
        let result =
            Self::check_position(jvm, this.clone(), line, direction).await?;
    ...
    async fn init_with_display(
        ...
        let formatter = jvm
            .new_class(
                "org/kwis/msp/lwc/TextFormatProcessor",
                "()V",
                (),
            )
            .await?;
