...
use jvm::{ClassInstanceRef, JavaChar, JavaError, Jvm, Result as JvmResult};
...
impl TextFieldComponentAction {
    pub fn as_proto() -> WieJavaClassProto {
        WieJavaClassProto {
            ...
            methods: vec![
                JavaMethodProto::new("<init>", "(Lorg/kwis/msp/lwc/TextFieldComponent;)V", Self::init, Default::default()),
                JavaMethodProto::new(
                    "action",
    ...
    async fn init(
        ...
        jvm.put_field(&mut this, "__wieOuter", "Lorg/kwis/msp/lwc/TextFieldComponent;", outer)
            .await?;
    ...
    async fn action(
        ...
        let outer: ClassInstanceRef<TextFieldComponent> = jvm.get_field(&this, "__wieOuter", "Lorg/kwis/msp/lwc/TextFieldComponent;").await?;
        ...
        if !jvm.is_instance(&**source, "org/kwis/msp/lwc/TextFieldComponent$TextPopup") {
            let exception = jvm.instantiate_class("java/lang/ClassCastException").await?;
            return Err(JavaError::JavaException(exception));
        }
        ...
        jvm.put_field(&mut outer, "__wieTextFieldVisibleEnd", "I", visible_end).await?;
        ...
        let current_text: ClassInstanceRef<String> = jvm.get_field(&outer, "text", "Ljava/lang/String;").await?;

        let char_count: i32 = if current_text.is_null() {
            0
        } else {
            jvm.invoke_virtual(&current_text, "length", "()I", ()).await?
        };
        ...
        if visible_end >= 0 {
            ...
            let viewport_width: i32 = jvm.get_field(&outer, "__wieTextFieldViewportWidth", "I").await?;

            let mut index = visible_end;
            let mut accumulated_width = 0i32;

            while index >= 0 {
                let ch: JavaChar = jvm.invoke_virtual(&current_text, "charAt", "(I)C", (index,)).await?;
                ...
                if accumulated_width > viewport_width {
                    jvm.put_field(&mut outer, "__wieTextFieldVisibleStart", "I", index.wrapping_add(1))
                        .await?;
                    break;
                }
        ...
        let shell: ClassInstanceRef<()> = jvm.get_field(&outer, "__wieTextFieldPopup", "Lorg/kwis/msp/lwc/ShellComponent;").await?;
        ...
        jvm.put_field(&mut outer, "__wieTextFieldPopup", "Lorg/kwis/msp/lwc/ShellComponent;", null_shell)
            .await?;
