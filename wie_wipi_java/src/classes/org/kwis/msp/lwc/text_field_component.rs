...
impl TextFieldComponent {
    ...
    async fn init_with_display(
        ...
        jvm.put_field(
            &mut this,
            "__wieTextFieldPreferredHeight",
            "I",
            font_height.wrapping_add(4),
        )
        .await?;
    ...
    async fn notify_change_position(
        jvm: &Jvm,
        mut this: ClassInstanceRef<TextFieldComponent>,
    ) -> JvmResult<()> {
        ...
        if caret < old_start {
            ...
            let new_end = caret
                .wrapping_add(old_end)
                .wrapping_sub(1)
                .wrapping_sub(old_start);
    ...
    async fn control_popup(
        ...
        // shell.useFrame(true)
        let current_shell: ClassInstanceRef<()> = jvm
            .get_field(
                &this,
                "__wieTextFieldPopup",
                "Lorg/kwis/msp/lwc/ShellComponent;",
        ...
        let title: ClassInstanceRef<String> =
            jvm.intern_string("Wide Editor").await?.into();
    ...
    async fn key_notify(
        ...
        if matches!(event_type, 1 | 2 | 3) {
            ...
            if action == 5 {
                ...
                if event_type == 2 {
                    let boundary_hit: i32 = jvm
                        .get_field(
                            &this,
                            "__wieTextFieldBoundaryHit",
                            "I",
                        )
                        .await?;
    ...
    async fn paint_content(
        ...
        let graphics_font: ClassInstanceRef<()> = jvm
            .invoke_virtual(
                &graphics,
                "getFont",
                "()Lorg/kwis/msp/lcdui/Font;",
        ...
        let field_width = text_width
            .wrapping_add(6)
            .min(width.wrapping_sub(1));
        ...
        let configured_bg: i32 =
            jvm.get_field(&this, "bg", "I").await?;

        let normal_fg = if configured_bg >= 0 {
            configured_bg
        } else {
            0x0000_0000i32
        };
        ...
        while index < char_count {
            ...
            let draw_ch: JavaChar = if constraint == 2 {
                42u16
            } else {
                ch
            };
            ...
            if inverse != 0 {
                ...
                let inverse_fg = if configured_bg >= 0 {
                    configured_bg
                } else {
                    0x00ff_ffffi32
                };
        ...
        if mask & 2 != 0 {
            ...
            if caret >= char_count
                || caret == visible_end.wrapping_add(1)
            {
                let font_height: i32 = jvm
                    .invoke_virtual(
                        &font,
